...
**Chroma Strata** (PoT-style skin lab): open `pages/chroma-strata.html` from the site nav or `http://127.0.0.1:8765/chroma-strata.html` when `serve_companion.py` is running. Loads **`pages/skin-db/`** (see `pages/skin-db/README.md`): colors **1–6** (color **5** tints the selected **pattern**; **6** = eyes), constants **background / lineart / details**, optional **Info** file. **Randomize colors only** vs **colors + patterns** (**Shift+R** / **R**). Custom hex swatches per slot persist in **localStorage**; commit PNGs under `skin-db/<Species>/<Skin>/` so assets load automatically. **Download PNG** exports the canvas.
