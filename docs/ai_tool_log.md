...
- Chroma Strata: **full-width preview hero**; **preview fit-only** (viewport wrapper, ~34vh cap, dark frame); **`tintComposite` multiply** for tint slots **1–4**, pattern, and **6** on gray base; **`constantsBlend`** lineart / infoLayer; draw order Color1→4 → pattern → Color6 → Details → Lineart → Background → Palette → Info; **AsagiYang** swatch hexes + patternTint `#401A11`, **one** pattern PNG in manifest.
