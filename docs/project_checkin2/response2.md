...
- **Latest model update:** Fine-tuned checkpoint from 300 PoT screenshots (`epoch=15`) with LR sweep. The accuracy pick is `lr=1e-4` (`0.767` validation accuracy on the 60-image split). Class weighting alone did not fix false negatives at the default threshold on the tiny 10-image holdout, so the agent safety operating point switched to `lr=3e-5`, predator class weight `3.0`, and threshold `0.20`, moving predator recall from `0.571` to `0.714` while keeping holdout accuracy at `0.70`.
