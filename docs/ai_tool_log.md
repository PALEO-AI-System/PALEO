...
- Reworked Chroma Strata + **`pages/skin-db/`**: `index.json` + per-skin `manifest.json` (example **Yang/AsagiYang**), optional `AsagiYang_Info.md`; PoT-style slots (pattern ×3, color 5 tint, color 6 eyes), **tintMask** vs **imageOptions**, dual randomize + **Shift+R**, custom colors in localStorage; `skin-db/README.md` for naming and tint-vs-PNG options.
