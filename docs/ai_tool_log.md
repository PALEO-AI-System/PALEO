...
- Chroma Strata: **`tintComposite` multiply** for colors 2–4 + pattern + color 6; **`constantsBlend`** for lineart / infoLayer; **full-width preview hero** above controls; **Palette** layer + **three patterns** in Asagi manifest; compose order Background → Palette → Info; **preview fit-only** with viewport wrapper (no empty scroll from transform layout), capped height (~34vh / 420px), dark gradient frame (no checker); **AsagiYang** `palette` hexes ordered by hue / lightness.
